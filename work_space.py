...
def transcribeAudioZh(path, modelName="base.en", language="en", srtFilePathAndName="VIDEO_FILENAME.srt"):
    ...
    for segment in segments:
        ...
        for word in segment.words:
            ...
            # 排除英文字母+. 情况
            if (finalWord[-1] in END_INTERPUNCTION and not (
                    finalWord[-1] == "." and len(finalWord) > 1 and finalWord[-2] in ENGLISH_AND_NUMBER_CHARACTERS)) \
                    or (subtitle is not None and len(subtitle.content) > 20):
                if not ((finalWord[-1] == "." and len(finalWord) > 1 and finalWord[
                    -2] in ENGLISH_AND_NUMBER_CHARACTERS) or (subtitle is not None and len(subtitle.content) > 20)):
...
def srtFileDeeplXTran(sourceFileNameAndPath, outputFileNameAndPath):
    srtContent = open(sourceFileNameAndPath, "r", encoding="utf-8").read()
    subGenerator = srt.parse(srtContent)
    subTitleList = list(subGenerator)
    contentList = []
    ...
    contentList = deepLXTranslate(contentList)
...
def merge_en_zh_srt(srtEnFileNameMergeAndPath, srtZhFileNameAndPath, merged_en_zh_file_name_path):
    '''
    desc: 合并英文 & 中文字幕
    '''
    en_subtitle_list = list(srt.parse(open(srtEnFileNameMergeAndPath, "r", encoding="utf-8").read()))
    sorted_en_subtitle_list = srt.sort_and_reindex(en_subtitle_list)
    en_sub_title_map = {subTitle.index: subTitle for subTitle in sorted_en_subtitle_list}

    zh_subtitle_list = list(srt.parse(open(srtZhFileNameAndPath, "r", encoding="utf-8").read()))
    sorted_zh_subtitle_list = srt.sort_and_reindex(zh_subtitle_list)
    ...
    for i in range(max(len(zh_sub_title_map), len(en_sub_title_map))):
        ...
        end = en_sub_title.end if zh_sub_title is None else zh_sub_title.end
        en_content = "" if en_sub_title is None else en_sub_title.content
        zh_content = "" if zh_sub_title is None else zh_sub_title.content
        content = en_content + '\n' + zh_content
        subtitle_list.append(srt.Subtitle(index, start, end, content))
...
def zhVideoPreview(videoFileNameAndPath, voiceFileNameAndPath, insturmentFileNameAndPath, srtFileNameAndPath,
...
if __name__ == "__main__":
    ...
    if paramDict["srt merge translate"]:
        try:
            ...
            if paramDict["srt merge translate tool"] == "deeplx":
                srtFileDeeplXTran(srtEnFileNameMergeAndPath, srtZhFileNameAndPath)
            elif paramDict["srt merge translate tool"] == "deepl":
